...
enum DragState {
    ...
    case dragging(index:Int? = nil,transition:CGSize)
...
struct WalletView: View {
                                ...
                                TapGesture()
                                    ...
                                    .updating(self.$dragState, body: { (value, state, transaction) in
                                        ...
                                        case .second(true, let drag):
                                            state = .dragging(index: self.index(for: card), transition: drag?.translation ?? .zero)
                                        default:
                                            break
                                        }
                                    })
    ...
    private func zIndex(for card:Card) -> Double{
        ...
        let defaltZIndex = -Double(cardIndex)
        
        if let draggingIndex = dragState.index,
           cardIndex == draggingIndex{
            return defaltZIndex + Double(dragState.translation.height/Self.cardOffset)
        }
        return defaltZIndex
    }
    ...
    private func offset(for card:Card) -> CGSize{
        ...
        return CGSize(width: 0 + pressedOffset.width, height: -50 * CGFloat(cardIndex)+pressedOffset.height
        + dragOffsetY)
    }
